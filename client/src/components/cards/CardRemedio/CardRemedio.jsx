...
export function CardRemedio({ medicamentoData, user }) {
      ...
      try {
         ...
         if (await changeStatusMedicamento(id, newStatus)) {
            showSuccessAlert({
               ...
               message:
                  "O medicamento não será mais exibido, porém a ação pode ser revertida acessando o card VISUALIZAR LIXEIRA",
            });
         }
      } catch (error) {
   ...
   const handleConfirmDelete = async (id) => {
      await showConfirmAlert({
         ...
         message:
            "Você tem certeza que deseja mover o Medicamento para a Lixeira? Esse medicamento não aparecerá mais, porém poderá ser restaurado posteriormente",
      ...
      <div className={styles.card} id={medicamentoData.id}>
         {user.nivel_acesso <= 1 && (
            <div className={styles.cardContent__btnAdminCollection}>
               ...
               <button
                  className={styles.btnAdminContent__btnDel}
                  onClick={async () => await handleConfirmDelete(medicamentoData.id)}
               >
                  <IconDel />
               </button>
            </div>
